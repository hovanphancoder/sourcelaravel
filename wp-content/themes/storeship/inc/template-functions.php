...
function storeship_body_classes($classes)
{
    ...
    if ($disable_sticky_header_option == true) {
        $classes[] = 'aft-no-sticky-header';
    }
        ...
        if ($post_categories) {
            ...
            foreach ($post_categories as $post_category) {
                ...
                $output .= '<li class="meta-category">
                             <a class="storeship-categories ' . esc_attr($color_class) . '" href="' . esc_url(get_term_link($post_category)) . '" alt="' . sprintf(esc_attr__('View all posts in %s', 'storeship'), $post_category->name) . '">
...
if (!function_exists('storeship_get_product_list')) :
     ...
     *
     */
    function storeship_get_product_list($number_of_products = 5, $category)
    {
