 ...
 */
if (!function_exists('storeship_remove_review_tab')) {
    /**
     * Product columns wrapper.
     *
     * @return  void
     */
    function storeship_remove_review_tab($tabs)
    {
        $review_tab = storeship_get_option('store_product_page_review_tab');
        if ($review_tab == 'no') {
            unset($tabs['reviews']);
...
add_filter('woocommerce_product_tabs', 'storeship_remove_review_tab', 98, 1);
...
function storeship_woocommerce_related_products_args($args)
{
    ...
    $posts_per_page = storeship_get_option('store_product_page_related_products_per_page');
