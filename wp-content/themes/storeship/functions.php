<?php
    ...
    if (!function_exists('storeship_fonts_url')):
        ...
        function storeship_fonts_url()
        {
            ...
            if ('off' !== _x('on', 'Roboto font: on or off', 'storeship')) {
                $fonts[] = 'Roboto:100,300,400,500,700';
            }

            /* translators: If there are characters in your language that are not supported by Lato, translate this to 'off'. Do not translate into your own language. */
            if ('off' !== _x('on', 'Lato font: on or off', 'storeship')) {
    ...
    function storeship_scripts()
    {
        ...
        $disable_sticky_header_option = storeship_get_option('disable_sticky_header_option');
        if ($disable_sticky_header_option == false) {
            //wp_enqueue_script('storeship-fixed-header-script', get_template_directory_uri() . '/assets/fixed-header-script.js', array('jquery'), '', true);
        }
    ...
    function storeship__admin_scripts($hook)
    {
        if ('widgets.php' === $hook) {
            wp_enqueue_script('jquery');
            wp_enqueue_media();
            wp_enqueue_script('storeship-widgets', get_template_directory_uri() . '/assets/widgets.js', array('jquery'), '1.0.0', true);
        }
    }
